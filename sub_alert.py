...
def reachable_check():
    try:
        ...
        is_reachable = True
        ips = server_ip.split("|")
        skip_ips = skip_ip.split("|")

        print("reachable_check (SSH):")
        for ip in ips:
            if ip in skip_ips:
                print(f"Skip {ip}")
                continue
            ...
            try:
                ...
                server_post(
                    machine_name,
                    str(ip)
                    + "，服务器不可达（SSH端口无法连接，可能宕机/网络故障），请及时排查！",
                )
                is_reachable = False
            finally:
                s.close()
        ...
        print("reachable_check error!", str(e))
